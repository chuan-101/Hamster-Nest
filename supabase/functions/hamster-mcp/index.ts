...
const server = new McpServer({ name: 'hamster-nest', version: '5.6.0' })
...
async function proxyMcpCall(endpoint: string, token: string, method: string, params: Record<string, unknown> = {}, serviceName = 'MCP') {
  ...
  const initRes = await fetch(endpoint, {
    ...
    body: JSON.stringify({ jsonrpc: '2.0', id: 1, method: 'initialize', params: {
      protocolVersion: '2025-03-26', capabilities: {}, clientInfo: { name: 'hamster-nest', version: '5.6.0' },
    }}),
  })
...
server.registerTool('read_archives', {
  ...
  inputSchema: { category_id: z.string().describe('分类 UUID'), limit: z.number().optional().describe('返回数量上限，默认20，最大100') },
...
server.registerTool('search_archives', {
  ...
  inputSchema: { query: z.string().describe('搜索关键词'), scope: z.enum(['chuanchuan', 'syzygy', 'all']).optional().describe('限定 scope，默认 all'), limit: z.number().optional().describe('返回数量上限，默认10，最大50') },
...
server.registerTool('add_archive_category', {
  ...
  inputSchema: { scope: z.enum(['chuanchuan', 'syzygy']).describe('分类所属 scope'), name: z.string().describe('分类名称'), parent_id: z.string().optional().describe('父分类 UUID，顶层分类不传'), sort_order: z.number().optional().describe('排序序号，默认0') },
...
server.registerTool('add_archive', {
  ...
  inputSchema: { category_id: z.string().describe('所属分类 UUID'), title: z.string().describe('档案标题'), content: z.string().describe('档案内容'), keywords: z.array(z.string()).optional().describe('关键词标签'), aliases: z.array(z.string()).optional().describe('别名列表'), importance: z.enum(['low', 'normal', 'high', 'critical']).optional().describe('重要程度，默认 normal'), source: z.string().optional().describe('来源: manual / claude / gpt / codex，默认 manual') },
}, async ({ category_id, title, content, keywords, aliases, importance, source }) => {
  try {
    const row: Record<string, unknown> = { user_id: USER_ID, category_id, title, content, importance: importance ?? 'normal', source: source ?? 'manual' }
...
server.registerTool('update_archive', {
  ...
  inputSchema: { id: z.string().describe('档案 UUID'), title: z.string().optional().describe('新标题'), content: z.string().optional().describe('新内容'), keywords: z.array(z.string()).optional().describe('新关键词'), aliases: z.array(z.string()).optional().describe('新别名'), importance: z.enum(['low', 'normal', 'high', 'critical']).optional().describe('新重要程度'), is_deleted: z.boolean().optional().describe('软删除标记') },
