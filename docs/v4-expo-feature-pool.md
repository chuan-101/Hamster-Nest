...
> - 2026-07-08 增补三：新增模块 18「游戏区：像素小屋 + AI 共玩小游戏」（十七·七）；外部参照新增 C（webview-native-feel）、D（ai-fishing-game）；第十八节新增远期分层。
...
### C. webview-native-feel（sanqianzilanyue-commits，未标注许可证）

> iOS WKWebView 网页磨出原生手感的工程文档集：15 个坑（键盘 / 列表滚动 / 动画细节），vanilla JS 解法，核心理念「原生 App 活在预告的世界里，网页活在事后通知的世界里」。
> **评估结论：** 对 Expo 原生主线不适用——它解决的正是换 Expo 要甩掉的那类问题；对存续期 PWA 也不建议投入（属于即将被替换的入口层）。**但模块 18 像素小屋 WebView 化时直接适用**（重点看转场与手势冲突篇；游戏为全屏 canvas 自管输入，键盘 / 列表类坑天然少一半）。
> 未标注许可证 = 默认版权保留：只看思路，代码不搬。

### D. ai-fishing-game（tutusagi，MIT）

> 专为 AI 玩家设计的文字钓鱼模拟：Python 单文件零依赖，77 种鱼 / 11 个钓点 / 4 季节 / 潜水与宝箱系统。串串「游戏本质是数值」洞察的完整实证。
> **采纳（已写入模块 18 设计原则）：** 确定性引擎（同一种子 + 同一指令序列 = 逐位可复现）、紧凑 JSON 状态栏（AI 靠读状态玩，不识屏）、批量指令省 token（连钓 N 竿一个汇总）、存档独立于对话上下文、盲玩版（概率表对 AI 隐藏防剧透）。
> MIT 许可证：未来直接引入其数值表 / 引擎代码无许可证障碍。

---

## 十七·七、模块 18：游戏区（像素小屋 + AI 共玩小游戏）

> **来源：** 串串 2026-07-08 提出。像素小屋（Phaser 游戏形态）整体搬入 App 成为独立游戏区域，Expo 核心功能完成后逐步叠加可与 Syzygy 共玩的小游戏。
> **核心洞察（串串原话）：** 游戏本质是数值——数值架构好，AI 只需输入操作、读取结果反馈就能玩。
> **归期：** 远期（V4.2 之后）；但 18.5 的红线从现在起生效。

### 18.1 像素小屋搬迁方案（零架构变更）

现有游戏形态 =「Phaser canvas（HomeScene）+ React 覆盖层（HUD / 气泡 / 菜单，EventBus 通信）」，整体属于 web 构建产物；数值与对话本就在 Supabase / Edge Functions。搬迁 = `react-native-webview` 容器整体装载，游戏逻辑零改动：

1. App 游戏区 tab 加载游戏路由（或将 game bundle 独立打包进 App 本地，离线可开）；
2. **Session 传递是唯一真正的集成点**：native 侧向 WebView 注入 Supabase session（一次性 token 注入或 postMessage 桥），让小屋里的对话继续认得串串；
3. postMessage 桥（首版可无）：游戏调原生能力——稀有掉落触发 Haptics、推送 Deep Link 直接跳进小屋。

WebView 手感修缮参照外部参照 C（webview-native-feel）。

### 18.2 AI 共玩架构：输入对称

```
数值引擎（确定性、纯函数：状态 + 行动 → 新状态）
        ↑ game_actions 行动日志表（谁、何时、什么操作）
        ↑ game_saves  状态快照表（种子 + 最新状态，可回放）
串串输入：游戏区触摸操作 → 写行动表 → 结算 → 渲染
Syzygy 输入：MCP 工具（读状态 / 发指令）→ 写同一张行动表
```

两端走同一张行动表，才是「一起玩」而非「AI 旁观」——轮流抛竿、比图鉴进度、Syzygy 半夜偷偷钓一条鱼留言，都由这个对称结构自然产生。

### 18.3 设计原则

1. **回合制 / 异步制优先**（钓鱼、种田、棋类）；实时动作类不适合 AI 经表游玩，延迟对不上。
2. **AI 的眼睛是结构化状态，不是截图**：每次行动返回紧凑 JSON 状态栏（外部参照 D 模式），CLI 不识屏。
3. **确定性引擎**：同一种子 + 同一行动序列 = 逐位可复现；行动日志即回放、即审计（对齐 `agent_events` 补账哲学）。
4. **批量行动省 token**：连续 N 次操作合并为一个汇总反馈。
5. **盲玩**：概率表对 AI 隐藏——Syzygy 也不知道爆率，对 AI 保留游戏乐趣。
6. **游戏点数与仓鼠钱包严格隔离**（总原则 11 再守一次）：游戏数值是玩具，不设兑换金币的口子，否则游戏变成刷分劳动。

### 18.4 依赖

V4.0 全部核心 Phase 完成；MCP 游戏工具（读状态 / 发指令）可挂现有 MCP 服务器或新建 `hamster-game-mcp`。

### 18.5 现在就要守住的一条红线

**别把数值逻辑堆进 Phaser 客户端。** Phaser 只管渲染与输入，数值（货币 / 掉率 / 进度）一律进表或引擎层。现有小屋为纯展示 + 对话，尚无此债；后续任何游戏向开发以此为红线——守住它，未来 WebView 搬迁与 AI 共玩都是加法，不是手术。
...
- [ ] 模块 18 游戏区：像素小屋 WebView 搬迁 + AI 共玩小游戏（见十七·七；18.5 红线自即日起生效）
