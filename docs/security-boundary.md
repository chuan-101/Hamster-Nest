...
- `anon` 角色 = 公网。anon key 随前端发布，等于公开常量，**任何策略都不得向 anon 授予读写**。
  唯一例外是 `device_status` 的 `Allow insert for anon`（iOS 快捷指令上报通道），
  待快捷指令切换到 `device-report` 函数后删除——这是当前 anon 面上最后一条策略。
...
| `letter-generate` | **A/B（2026-07-06 修复）** | 50/天 | 曾有 eyJ 前缀兜底 + serviceKeyPrefix 日志泄漏，均已删除 |
| `letter-check` | **A/B（2026-07-06 修复）** | – | 同上；本身不调模型 |
...
- **0-2 收尾**：`device_status` 的 `Allow insert for anon` 待 iOS 快捷指令切到 `device-report` 并验证落数后删除。
